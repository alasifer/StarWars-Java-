...
public class Attack extends SWAffordance implements SWActionInterface {
	...
	public void act(SWActor a) {
		...
		boolean targetIsEntity = target instanceof SWEntity; // to check if the the target is an entity , so that
																// water-reservoir can can be attacked

		SWActor targetActor = null;
		SWEntity targetEntity = null;
		int energyForAttackWithWeapon = 1;//the amount of energy required to attack with a weapon

		if (targetIsActor) {
			targetActor = (SWActor) target;
		}
		else if (target instanceof SWEntity){
			targetEntity = (SWEntity) target;
		}
		...
		if (targetIsActor && (a.getTeam() == targetActor.getTeam())) { //don't attack SWActors in the same team
			...
			if (itemCarried != null) {//if the actor is carrying an item
				if (itemCarried.hasCapability(Capability.WEAPON)) {
					if (itemCarried.getSymbol() == "†") { // check if the item is a lightsaber
						int chkforce = a.getForceAbility().getForce();
						if (chkforce < 60) { // check if the actor has enough force to wield a lightsaber
							a.say("\t" + a.getShortDescription() + " says: Ooops not enough force");
							return;
						}
					}
					...
					if (targetIsActor) {
						targetActor.say("\t" + targetActor.getShortDescription() + " is amused by " + a.getShortDescription() + "'s attempted attack with " + itemCarried.getShortDescription());

					}
				}
			}
			else if ((targetIsActor) || (targetIsEntity)){ // attack with bare hands
			...
			if (this.getTarget().getHitpoints() <= 0) {  // can't use isDead(), as we don't know that the target is an actor
				...
				if (targetIsActor) {
					...
					target.setLongDescription(target.getLongDescription());
