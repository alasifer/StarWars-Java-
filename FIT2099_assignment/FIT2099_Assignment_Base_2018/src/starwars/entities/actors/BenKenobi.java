...
import starwars.actions.Training;
...
public class BenKenobi extends SWLegend {
	...
	private Patrol path;
	private boolean trainingMode; // this helps to determine whether ben should move or not
	...
	private BenKenobi(MessageRenderer m, SWWorld world, Direction [] moves) {
		...
		setForce(new Force(100));
		trainingMode = false;
		addAffordance(new Training(this, m));
	}
	...
	@Override
    /**
     * this method preform all possible action that ben can do
     *
     * if ben is not training luke he will move randomly
     */
